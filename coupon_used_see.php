...
$image=mysql_query("SELECT * FROM members WHERE username='$user'");
			...
			<font id="inde_admin_signup"><?php echo $_GET["action"]; ?>&nbsp;Used Coupon Cards</font>
			</div>
		...
		<div class="navbar-inner">
							...
							<center>
								<ul id="myTab" class="">
								<a href="home.php?Ha_Account=" class="blue"><span>Back To Unused Coupon</span></a>
					...
					<?php 
						echo '<a href="coupon_used_see.php?action='.$row["coupon_price_entry"].'" class="blue" >';
	...
	echo '<div class="alert alert-dismissable alert-success"><strong>';
		echo "All Used"." ".$_GET["action"]." "."Birr Coupon Card Successfully Deleted !";
		echo '</strong></div>';
 ...
 $chek_user = "select * from  account_card where card_name='$card_name'  ";
	...
	mysql_query("insert into account_card (card_name,card_number,card_price,card_status,date)
	values('$card_name','$card','$card_price','Unused',curdate() )	")or die(mysql_error());
	}
	echo '<div class="alert alert-dismissable alert-success"><strong>';
            ...
            <div class="container">
					<form method="post" action="" >
                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" width="100%" id="example">
							...
							<?php 
							$coupon_price_entry = $_GET["action"];
							$query=mysql_query("select * from   account_card where card_price='$coupon_price_entry' and card_status='Used' order by card_id DESC") or die(mysql_error());
							$count = mysql_num_rows($query);
							if($count == 0){
								echo '<div class="alert alert-dismissable alert-error">';
									echo "Used"." ".$coupon_price_entry." "."Birr Coupon Note Available !";
								echo '</div>';
							}
							$i=0;
						...
						<table ><tr>
							...
							$query=mysql_query("select * from  account_card where card_price='$coupon_price_entry' and card_status='Used'")or die(mysql_error());
							...
							?>
											...
											echo '<form method="POST" action=""  onclick="return confirmdelete();" >';
												...
												echo '<input  type="submit" class="btn btn-warning"  name="delete_all_used" value="'."Delete All Used"." ".$_GET["action"]." "."Coupon Card".'" >'; 
											echo '</form>';
