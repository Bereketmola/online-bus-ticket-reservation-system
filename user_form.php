    ...
    <div class="modal-header">
		<font  class="btn  btn-large" id="inde_admin_signup"> User</font>
    </div>	
...
if (isset($_POST['user_login'])) 
	...
	$count = 0;
		...
		$result = mysql_query("SELECT * FROM members
				WHERE ((`members`.`username` = '$username') AND (`members`.`password` = '$password')AND (`members`.`member_type` = 'user')  )");
...
?>	
			...
			<form method="POST" action="" id="for">
				...
				<input type="hidden" title="Enter Your password" name="member_type"  value="user" required>
				 <tr>
                	<td>Password</td>
                    <td>
				   ...
				   <td></td>
            <td colspan="2">
                	<input type="submit" name="user_login"  value="Login" id="kl" class="btn">
                </td>
            </tr>
		<tr>
		</form>
