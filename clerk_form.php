    ...
    <div class="modal-header">
		<font  class="btn  btn-large" id="inde_admin_signup"> Clerk</font>
    </div>	
...
if (isset($_POST['clerk_login'])) 
  	{	
		...
		$result = mysql_query("SELECT * FROM members
				WHERE ((`members`.`username` = '$username') AND (`members`.`password` = '$password')AND (`members`.`member_type` = 'clerk')  )");
...
?>
			...
			<form method="POST" action="" id="for">
				...
				<input type="hidden" title="Enter Your password" name="member_type"  value="clerk" required>
				 <tr>
                	<td>Password</td>
                    <td>
				   ...
				   <td></td>
            <td colspan="2">
                	<input type="submit" name="clerk_login"  value="Login" id="kl" class="btn">
                </td>
            </tr>
		<tr>
		</form>
